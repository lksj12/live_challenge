...
export function createNotesRouter(database) {
    ...
    router.post('/', (request, response) => {
        ...
        database
          ...
          updated_at
            ...
            .run(id, ownerId, parsed.data.title, parsed.data.content, color, priority, sizeBytes, now, now);
    ...
    router.patch('/:noteId', (request, response) => {
        ...
        database
         ...
         WHERE id = ? AND owner_id = ?`)
            .run(updated.title, updated.content, updated.color, updated.priority, updated.isPinned ? 1 : 0, updated.status, getNoteSize(updated.title, updated.content), updated.updatedAt, noteId, ownerId);
        response.json({
    ...
    router.put('/:noteId/tags', (request, response) => {
        ...
        const replaceTags = database.transaction(() => {
            ...
            const insert = database.prepare(`INSERT INTO note_tags (note_id, tag_id, created_at)
         VALUES (?, ?, ?)`);
            const now = new Date().toISOString();
...
function findOwnedNote(database, noteId, ownerId) {
    const row = database
        .prepare('SELECT * FROM notes WHERE id = ? AND owner_id = ?')
        .get(noteId, ownerId);
    return row ?? null;
}
function mapNote(database, row) {
    const tags = database
        .prepare(`SELECT tags.name
       FROM tags
...
function parseNoteId(value, response) {
...
function getNoteSize(title, content) {
