...
export function createAdminRouter(database) {
    ...
    router.post('/users/:userId/reset-password', async (request, response) => {
        ...
        const resetPassword = database.transaction(() => {
            ...
            database
             ...
             WHERE user_id = ?`)
                .run(now, userId);
            insertAuditLog(database, request.auth.user.id, userId, 'USER_PASSWORD_RESET', { email: target.email }, now);
        });
        resetPassword();
    ...
    router.delete('/users/:userId', (request, response) => {
        ...
        const deleteUser = database.transaction(() => {
            insertAuditLog(database, request.auth.user.id, userId, 'USER_ACCOUNT_DELETED', { email: target.email }, now);
            database
...
function parseUserId(value, response) {
...
function findManagedUser(database, userId) {
    const user = database
        .prepare("SELECT id, email FROM users WHERE id = ? AND role = 'user'")
        .get(userId);
    return user ?? null;
}
...
function insertAuditLog(database, actorUserId, targetUserId, action, details, createdAt) {
    database
        .prepare(`INSERT INTO audit_logs (
        id, actor_user_id, target_user_id, action, details_json, created_at
      ) VALUES (?, ?, ?, ?, ?, ?)`)
        .run(randomUUID(), actorUserId, targetUserId, action, JSON.stringify(details), createdAt);
}
...
function respondUserNotFound(response) {
