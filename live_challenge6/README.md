...
| 화면 | 파일·링크 | 설명 |
|---|---|---|
| 상품 목록·검색 | [](react-shopping-mall/screenshots/01.product_list_category.png) | Fake Store API 상품 목록, 상품명 검색, 카테고리 필터 및 재고 상태 |
| 로그인·회원가입 | [](react-shopping-mall/screenshots/02.login_register.png) | 자체 회원가입·로그인 화면과 로그인 오류 안내 |
| 상품 상세·재고 | [](react-shopping-mall/screenshots/03.product_detail.png) | 상품 상세 정보, 저재고·품절 표시 및 재입고 알림 신청 |
| 장바구니·결제 | [](react-shopping-mall/screenshots/04.cart.png) | 상품 수량 변경, 삭제, 총 결제 예정 금액, 재고 부족 안내 및 모의 결제 |
| 계정 관리 | [](react-shopping-mall/screenshots/05.account_management.png) | 사용자 정보, 비밀번호 변경 및 회원 탈퇴 기능 |
| 구매 내역 | [](react-shopping-mall/screenshots/06.order_history.png) | 주문별 결제 상태, 배송 상태, 주문 상품과 한국 시간 기준 주문 일시 |
| 관리자 사용자 관리 | [](react-shopping-mall/screenshots/07.admin_account.png) | 사용자 목록, 임시 비밀번호 초기화 및 일반 사용자 삭제 |
| 관리자 상품 관리 | [](react-shopping-mall/screenshots/08.admin_product.png) | 상품별 현재 재고 조회, 재고 수량 변경 및 품절 상태 관리 |

> 실행 화면에는 실제 비밀번호, JWT, 개인정보 또는 실제 결제 정보가 노출되지 않도록 하였다. 테스트 계정의 이메일이 표시되는 경우 과제용 계정만 사용하였다.

### 실시간 응시와 최종 보완 비교

1시간 종료 시점에는 프로젝트 구조, 데이터 구조, 전역 상태와 인증 방식을 중심으로 설계하였다. 최종 제출 단계에서는 설계를 실제 코드로 구현하고, 기능 테스트 과정에서 발견한 상태 동기화와 재고 처리 문제를 추가로 수정하였다.

| 항목 | 1시간 종료 시 | 최종 제출 시 | 보완 내용 |
|---|---|---|---|
| 데이터·상태·인증 설계 | React, Redux Toolkit, Express, SQLite, JWT 기반 구조 설계 | 상품, 인증, 장바구니, 재고 상태와 API를 기능별로 분리하여 구현 | 사용자 역할, 임시 비밀번호 변경 상태와 재고 상태 추가 |
| 전역 장바구니 | 상품과 수량을 저장하는 Redux 상태 구조 설계 | 상품 추가, 수량 변경, 삭제, 전체 삭제, 총액 계산 및 사용자별 저장 구현 | 비회원 장바구니 이전, 회원 장바구니 병합·덮어쓰기 및 `ownerId` 기반 저장 추가 |
| 사용자 인증 | Express, SQLite, bcrypt, JWT 기반 인증 방식 설계 | 회원가입, 로그인, 인증 상태 복원, 로그아웃, 비밀번호 변경 및 회원 탈퇴 구현 | 관리자 권한, 임시 비밀번호 초기화와 강제 변경 기능 추가 |
| 상품 API·대체 경로 | Fake Store API 사용 및 실패 처리 방식 설계 | API 연동, 응답 검증, `loading`, `error`, `empty` 및 로컬 mock 대체 구현 | 이미지 실패 대체, 상품 검색, 카테고리 필터와 상품 상세 페이지 추가 |
| 상품 재고 | 상품별 고정 재고 표시 방식 검토 | SQLite 재고 테이블과 공용 재고 API 구현 | 관리자 재고 변경, 저재고·품절 표시, 재입고 알림과 장바구니 재고 검증 추가 |
| 주문·모의 결제 | 초기 범위에 포함하지 않음 | 로그인 사용자 모의 결제, 주문 저장 및 구매 내역 조회 구현 | 결제 직전 서버 재고 재검사, SQLite 트랜잭션 기반 재고 차감과 실패 시 롤백 추가 |
| 관리자 기능 | 초기 범위에 포함하지 않음 | 사용자 관리와 상품 재고 관리 화면 구현 | 관리자 계정 보호, 사용자 비밀번호 초기화 및 일반 사용자 삭제 추가 |
| README·테스트 | 제출 문서 구성과 테스트 항목 설계 | README, AI 활용 기록, 오류 해결 기록과 기능 테스트 결과 작성 | TypeScript 빌드 오류, 장바구니 저장, 재고 동기화, 결제 버튼과 시간대 오류 해결 과정 반영 |
...
| 운영 배포 미구성 | 로컬 과제 제출을 기준으로 구현함 | 클라이언트·서버·DB 배포 및 환경변수 구성 | 중간 |
